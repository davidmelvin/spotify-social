...
exports.getFollowedAccountsForUserID = async function foo(
  webAccessToken,
  userID
) {
  const res = await fetch(
...
exports.wrapWebApi = function wrapWebApi(api) {
  ...
  api.getFollowedAccountsOfUser = function getFollowedAccountsOfUser(
    userId,
    callback
  ) {
    return Request.builder()
      ...
      .withAuth(this.getAccessToken())
      .withPath(
        `/user-profile-view/v3/profile/${userId}/following?market=from_token`
      )
      .build()
      .execute(HttpManager.get, callback);
  };
